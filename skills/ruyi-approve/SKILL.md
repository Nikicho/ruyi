...
- 项目必须已初始化。
- 必须存在对应 test。
- test 必须锚定 contract 和 plan。
- test 的 `result` 必须是 `passed` 或 `passed-with-notes`。
- test 的 `approval` 必须是 `pending`。

## 3. 执行原则

- 审批关注交付是否可接受。
- 审批不替代代码 review，也不替代测试验证。
- 审批结论更新 test 的头部元信息，并在正文末尾写入 `## 审批结论`。
...
4. 确认 test 对应的 contract、plan、验证摘要和风险项。
5. 记录审批结论。
...
确认 test 已经通过，并且用户已经给出明确审批意见后，可以使用脚本记录审批结论：
...
- 校验项目已初始化。
- 校验 test 存在。
- 校验 test 锚定 contract 和 plan。
- 校验 test 已通过或带说明通过。
- 只更新 `approval: pending` 的 test。
- 不修改 contract、plan 或 spec。
